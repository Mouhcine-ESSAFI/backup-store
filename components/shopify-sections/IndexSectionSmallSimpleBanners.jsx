// components/shopify-sections/IndexSectionSmallSimpleBanners.jsx
// Converted from Shopify section: index-section-small_simple_banners

'use client';
import Image from 'next/image';

export default function IndexSectionSmallSimpleBanners({ settings, data }) {
  ...
  // Font size: range
  // Line height: range
  // Font size mobile: range
  // Line height mobile: range
  // Roundness: range
  // Banners height: range
  // Font weight: range
  // Change image size: checkbox
  // Image height on desktop: range
  // Image height on mobile: range
  // undefined: header
  ...
  // undefined: header
  // Center bottom: checkbox
  // Padding from top: checkbox
  // Bottom padding: text
  // undefined: header
...
<div className="{item.top_offset == 'none' && (nomargin {% elsif item.top_offset == 'small' %}tt-offset-small )}container-indent {item.custom_classes} smbsection{section.id}{item.usebgcolor && ( layout-box01-wrapper)}">
  <div className="container{item.fullwidth && (-fluid-custom)}">
    {%- if item.show_title -%}
    ...
    {%- endif -%}
    <div className="row minus20marg">
      {%- if section.blocks.size > 0 -%}
      {%- for block in section.blocks -%}{%- assign b_i = block.settings -%}
      
      <div className="{{ grid }}">
        <a href="{b_i.link}" className="small-info-bn small-info-bn{block.id}{b_i.image != blank && ( small-info-bn-img)}">
          <div>{b_i.text}</div>
        </a>
      </div>
