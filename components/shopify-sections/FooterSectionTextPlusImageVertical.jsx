// components/shopify-sections/FooterSectionTextPlusImageVertical.jsx
// Converted from Shopify section: footer-section-text_plus_image_vertical

'use client';
import Image from 'next/image';

export default function FooterSectionTextPlusImageVertical({ settings, data }) {
...
<div className="{template == 'index' && ({item.top_offset}{% else %}{item.top_offset_normal})} tpivsection{section.id} for-footer-blocks">
    ...
    <div className="row {item.contentalign}">
      <div className="col-md-{item.mdsize} col-lg-{item.lgsize} text-{item.textalign}">
        <div className="tt-layout-box03">
          ...
          {%- case block.type -%}
          {%- when 'image' -%}
          <div className="blid{block.id} respimgsize">
          ...
          <div className="row{b_i.textalign != "default" && ( text-{b_i.textalign})} blid{block.id}">
            <div className="col-12 col-sm-{{ b_i.columnswidth | split: "_" | first | split: "." | first }} col-lg-{{ b_i.columnswidth | split: "_" | first | split: "." | last }} blidfirstcol">
              {b_i.usetextastitle && (<h2 className="customtitle">)}
              {b_i.text}
              {b_i.usetextastitle && (</h2>)}
            </div>
            <div className="col-12 col-sm-{{ b_i.columnswidth | split: "_" | last | split: "." | first }} col-lg-{{ b_i.columnswidth | split: "_" | last | split: "." | last }} blidsecondcol">
              {b_i.text2}
            </div>
          </div>
          {%- else -%}
          <div className="blid{block.id}{b_i.textalign != "default" && ( text-{b_i.textalign})}">{b_i.text}</div>
          {%- endif -%}
          {%- when 'button' -%}
