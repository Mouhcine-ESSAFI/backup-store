// components/shopify-sections/HeaderTemplate2.jsx
// Converted from Shopify section: header-template-2

'use client';
import Image from 'next/image';

export default function HeaderTemplate2({ settings, data }) {
  // Settings from Shopify:
  // undefined: header
  // undefined: paragraph
  // Show categorie button: checkbox
  // undefined: header
  // Enable header underline: checkbox
  ...
  // Search indent. Work with enabled "Simple info": text
...
<header className="desctop-menu-large small-header headertype2{item.header_border && ( headerunderline)}">
  ...
  <div className="tt-desktop-header{settings.header_position and template == 'index' && ( tt-header-static)}">    
    ...
    <div className="container{item.header_fullwidth && (-fluid)}">
      <div className="tt-header-holder">
        ...
        <div className="search-type2{% unless item.show_simple_info %} obj-move-right{% endunless %} headerformplaceholderstyles">
          <!-- tt-search -->
          <form action="/search" method="get" role="search" style="margin-left: {item.search_indent}px;">
            <i className="icon-f-85"></i>
            <input type="hidden" name="type" value="product" />
            <input className="tt-search-input"
                   type="search"
                   ...
                   placeholder="{{ 'general.search.place_holder' | t }}"
                   aria-label="{{ 'general.search.place_holder' | t }}">
            <button type="submit" className="tt-btn-search">{{ 'general.search.button_name_header_type_2' | t }}</button>
          </form>
          <!-- /tt-search -->
        </div>
    ...
    <div className="container{item.header_fullwidth && (-fluid)} single-menu">
      <div className="tt-header-holder">
        <div className="tt-obj-menu obj-aligment-left">
          {%- if item.show_categorie_button -%}
          <div className="categories-btn-box tostuck-det-js" data-parent="categories-btn-box" data-stuckparent="tt-stuck-parent-categories">
        ...
        <div className="tt-col-obj tt-obj-options obj-move-right">
          <div className="tt-desctop-parent-search tt-parent-box hide"></div>
          {%- include "menu-cart" -%}
