// components/shopify-sections/FooterSectionSubscribe.jsx
// Converted from Shopify section: footer-section-subscribe

'use client';
import Image from 'next/image';

export default function FooterSectionSubscribe({ settings, data }) {
...
<div className="{template == 'index' && ({item.top_offset}{% else %}{item.top_offset_normal})} subsection{section.id} for-footer-blocks">
    ...
    <div className="row justify-content-center{item.usebgcolor and item.useboxedbgcolor && ( subsectionbg)}"{item.usebgcolor and item.useboxedbgcolor and item.showbgimage_bg && ( style="background-image: url({{ item.image_bg | img_url: '2048x' }});")}>
      <div className="col-md-{item.mdsize} col-lg-{item.lgsize}">
        <div className="tt-layout-newsletter02{item.allinleft && ( text-left)}">
          {item.text1 != "" && (<h5 className="tt-title">{item.text1}</h5>)}
