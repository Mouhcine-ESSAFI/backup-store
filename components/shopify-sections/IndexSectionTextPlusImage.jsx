// components/shopify-sections/IndexSectionTextPlusImage.jsx
// Converted from Shopify section: index-section-text_plus_image

'use client';
import Image from 'next/image';

export default function IndexSectionTextPlusImage({ settings, data }) {
  // Settings from Shopify:
  // Top Offset: select
  // Fullwidth size: checkbox
  // undefined: header
  // Show image: checkbox
  // Image: image_picker
  // Image style classes: text
  // Text 1: textarea
  // Text 2: textarea
  // undefined: header
  // Show button: checkbox
  // Use button as link: checkbox
  // Underline by default: checkbox
  // Button as link font size: text
  // Button as link font weight: text
  // Button text: text
  // Button link: url
  // Background color(underline): color
  // Text color: color
  // Background active color(underline): color
  // Text active color: color
  // Button custom styles: textarea
  // HOVER. Button custom styles: textarea
  // undefined: header
  // Reverse content: checkbox
  // Grid for: Image / Text: select
  // Content vertical align: select
  // Text align: select
  // undefined: header
  // Texts offset: text
  // Font size text 1: range
  // Line height text 1: range
  // Font weight text 1: range
  // Text 1 bottom padding: range
  // Font size text 2: range
  // Line height text 2: range
  // Font weight text 2: range
  // Button padding: range
  // undefined: header
  // Text 1 size mobile: range
  // Line height text 1 mobile: range
  // Text 1 bottom padding mobile: range
  // Text 2 size mobile: range
  // Line height text 2 mobile: range
  // Button padding mobile: range
  // undefined: header
  // Text 1 color: color
  // Text 2 color: color
  // undefined: header
  // Use background color: checkbox
  // Background color: color
  // Vertical top padding desktop: text
  // Vertical bottom padding desktop: text
  // Vertical top padding mobile: text
  // Vertical bottom padding mobile: text
  // undefined: paragraph
  // Show background image: checkbox
  // Image: image_picker
  // Image postion: select
  // Image size: select
  // Use minimal height: checkbox
  // Min height for big screen size: text
  // Min height for medium screen size: text
  // Min height for mobile screen size: text

  return (
    <>
      {%- assign item = section.settings -%}

<div className="{item.top_offset == 'none' && (nomargin {% elsif item.top_offset == 'small' %}tt-offset-small )}container-indent tpisection{section.id}">
  {item.usebgcolor && (<div className="layout-box01-wrapper"{item.showbgimage_bg && ( style="background-image: url({{ item.image_bg | img_url: '2048x' }});")}>)}
    <div className="container{item.fullwidth && (-fluid)}">
      <!-- obj -->
      <div className="row tt-layout-box01{item.reverse && ( flex-row-reverse)} {item.contentalign}">
        <div className="col-img col-md-{{ item.grid | split: '.' | first }}{% unless item.showimage %} d-sm-none-custom{% endunless %} respimgsize{item.imageclasses && ( {item.imageclasses})}">
          {item.showimage && (
          {%- if item.image == blank -%}{%- include "get_svg" type:'image' size:'580x400' -%}
          {%- else -%}
          <img className="lazyload"
               data-mainimage="{{ item.image | img_url: '100x100' | replace: '100x100', 'respimgsize' }}"
               alt="{{ item.image.alt }}">
          {%- endif -%}
          )}
        </div>
        <div className="col-description col-md-{{ item.grid | split: '.' | last }} text-{item.textalign}">
          {%- if item.text1 != "" -%}{item.text1}{%- endif -%}
          {%- if item.text2 != "" -%}{item.text2}{%- endif -%}
          {%- if item.showbtn -%}
          {%- if item.showbtn_aslink -%}        
          <a href="{item.btnurl}"
             className="hover-underline{item.underlinebydefault && ( underlinebydefault)} btn-section-css"
             >{item.btntext}</a>
          {%- else -%}
          <a href="{item.btnurl}"
             className="btn btn-section-css"
             style="background:{item.bg_color}; color:{item.cbtntext};" data-bgc="{item.bg_color}" data-abgc="{item.bg_color_a}" data-c="{item.cbtntext}" data-ac="{item.cbtntext_a}" data-hovercolors
             >{item.btntext}</a>
          {%- endif -%}
          {%- endif -%}
        </div>
      </div>
      <!-- /obj -->
    </div>
  {item.usebgcolor && (</div>)}
</div>

<style>
  .tpisection{section.id} .tt-layout-box01 .col-description{
    font-weight: {item.fontweighttext2};
    color: {item.colortext2};
  }
  .tpisection{section.id} .tt-layout-box01 .col-description .tt-title{
    font-weight: {item.fontweighttext1};
    color: {item.colortext1};
  }
  @media (min-width: 1025px){
    .tpisection{section.id} .tt-layout-box01 .col-description{
      font-size: {item.fontsizetext2}px;
      line-height: {item.lineheighttext2}px;
    }
    .tpisection{section.id} .tt-layout-box01 .col-description .tt-title{
      font-size: {item.fontsizetext1}px;
      line-height: {item.lineheighttext1}px;
      padding: 0 0 {item.bottompaddingtext1}px 0;
    }
  }
  @media (max-width: 1024px){
    .tpisection{section.id} .tt-layout-box01 .col-description{
      font-size: {item.fontsizetext2_mobile}px;
      line-height: {item.lineheighttext2_mobile}px;
    }
    .tpisection{section.id} .tt-layout-box01 .col-description .tt-title{
      font-size: {item.fontsizetext1_mobile}px;
      line-height: {item.lineheighttext1_mobile}px;
      padding-bottom: {item.bottompaddingtext1_mobile}px;
    }
  }
  .tpisection{section.id} .layout-box01-wrapper{
    background-color: {item.bgco};
    background-repeat: no-repeat;
    background-size: {item.imagesize_bg};
    background-position: {item.imagealign_bg};
  }
  .tpisection{section.id} .tt-layout-box01:not(.flex-row-reverse) .col-description{
    padding-left: {item.textoffset}px;
  }
  .tpisection{section.id} .tt-layout-box01.flex-row-reverse .col-description:last-child{
    padding-right: {item.textoffset}px;
  }
  
  {item.btn_custom_styles != '' && (
  .tpisection{section.id} .tt-layout-box01 .col-description .btn{
    {item.btn_custom_styles}
  }
  )}
  {item.btn_custom_styles_hover != '' && (
  .tpisection{section.id} .tt-layout-box01 .col-description .btn:hover{
    {item.btn_custom_styles_hover}
  }
  )}
  @media (min-width: 1025px){
    .tpisection{section.id} .tt-layout-box01 .col-description .btn-section-css{
      margin-top: {item.buttonpadding}px;
    }
    .tpisection{section.id} .layout-box01-wrapper{
      padding-top: {item.bgdepad}px;
      padding-bottom: {item.bgdepad_2}px;
    }
  }
  @media (max-width: 1024px){
    .tpisection{section.id} .tt-layout-box01 .col-description .btn-section-css{
      margin-top: {item.buttonpadding_mobile}px;
    }
    .tpisection{section.id} .layout-box01-wrapper{
      padding-top: {item.bgmopad}px;
      padding-bottom: {item.bgmopad_2}px;
    }
  }
  {item.usemiheight && (
  @media (min-width: 791px){
    .tpisection{section.id} > div > div > .row{
      min-height: {item.backgroundminheight2}px;
    }
  }
  @media (min-width: 1200px){
    .tpisection{section.id} > div > div > .row{
      min-height: {item.backgroundminheight3}px;
    }
  }
  @media (max-width: 790px){
    .tpisection{section.id} > div > div > .row{
      min-height: {item.backgroundminheight1}px;
    }
  }
  )}
  
  .tpisection{section.id} .tt-layout-box01 a.hover-underline{
    font-size: {item.linkbtnsizetext}px;
    line-height: {item.linkbtnsizetext}px;
    font-weight: {item.linkbtnweighttext};
    color: {item.cbtntext};
    -webkit-transition: color .2s ease-out;
    transition: color .2s ease-out;
  }
  .tpisection{section.id} .tt-layout-box01 a.hover-underline:before{
    background: {item.bg_color_a};
    bottom: -1px;
  }
  .tpisection{section.id} .tt-layout-box01 a.hover-underline.underlinebydefault:before{
    width: 100%;
    background: {item.bg_color};
    -webkit-transition: all .3s ease-out;
    transition: all .3s ease-out;
  }
  .tpisection{section.id} .tt-layout-box01 a.hover-underline:hover{
    color: {item.cbtntext_a};
  }
  .tpisection{section.id} .tt-layout-box01 a.hover-underline.underlinebydefault:hover:before{
    width: 0%;
    background: {item.bg_color_a};
  }
  
</style>
    </>
  );
}

// Schema from Shopify
export const schema = {
  "name": "Text Plus Image",
  "class": "index-section",
  "settings": [
    {
      "type": "select",
      "id": "top_offset",
      "label": "Top Offset",
      "default": "normal",
      "options": [
        {
          "value": "none",
          "label": "None"
        },
        {
          "value": "small",
          "label": "Small"
        },
        {
          "value": "normal",
          "label": "Normal"
        }
      ]
    },
    {
      "type": "checkbox",
      "id": "fullwidth",
      "label": "Fullwidth size",
      "default": false
    },
    {
      "type": "header",
      "content": "Content"
    },
    {
      "type": "checkbox",
      "id": "showimage",
      "label": "Show image",
      "default": true
    },
    {
      "type": "image_picker",
      "id": "image",
      "label": "Image"
    },
    {
      "type": "text",
      "id": "imageclasses",
      "label": "Image style classes"
    },
    {
      "type": "textarea",
      "id": "text1",
      "label": "Text 1",
      "default": "A great about us block helps builds trust between you and your customers."
    },
    {
      "type": "textarea",
      "id": "text2",
      "label": "Text 2",
      "default": "The more content you provide about you and your business, the more confident people will be when purchasing from your store."
    },
    {
      "type": "header",
      "content": "Button"
    },
    {
      "type": "checkbox",
      "id": "showbtn",
      "label": "Show button",
      "default": false
    },
    {
      "type": "checkbox",
      "id": "showbtn_aslink",
      "label": "Use button as link",
      "default": false
    },
    {
      "type": "checkbox",
      "id": "underlinebydefault",
      "label": "Underline by default",
      "default": true
    },
    {
      "type": "text",
      "id": "linkbtnsizetext",
      "label": "Button as link font size",
      "default": "18",
      "info": "Integer. Value in px"
    },
    {
      "type": "text",
      "id": "linkbtnweighttext",
      "label": "Button as link font weight",
      "default": "400"
    },
    {
      "type": "text",
      "id": "btntext",
      "label": "Button text",
      "default": "SHOP NOW"
    },
    {
      "type": "url",
      "id": "btnurl",
      "label": "Button link"
    },
    {
      "type": "color",
      "id": "bg_color",
      "label": "Background color(underline)",
      "default": "#191919"
    },
    {
      "type": "color",
      "id": "cbtntext",
      "label": "Text color",
      "default": "#ffffff"
    },
    {
      "type": "color",
      "id": "bg_color_a",
      "label": "Background active color(underline)",
      "default": "#ffffff"
    },
    {
      "type": "color",
      "id": "cbtntext_a",
      "label": "Text active color",
      "default": "#191919"
    },
    {
      "type": "textarea",
      "id": "btn_custom_styles",
      "label": "Button custom styles",
      "info": "not for \"Underline by default\""
    },
    {
      "type": "textarea",
      "id": "btn_custom_styles_hover",
      "label": "HOVER. Button custom styles",
      "info": "not for \"Underline by default\""
    },
    {
      "type": "header",
      "content": "Settings"
    },
    {
      "type": "checkbox",
      "id": "reverse",
      "label": "Reverse content",
      "default": false
    },
    {
      "type": "select",
      "id": "grid",
      "label": "Grid for: Image / Text",
      "default": "6.6",
      "options": [
        {
          "value": "6.6",
          "label": "6 / 6"
        },
        {
          "value": "7.5",
          "label": "7 / 5"
        },
        {
          "value": "5.7",
          "label": "5 / 7"
        },
        {
          "value": "8.4",
          "label": "8 / 4"
        },
        {
          "value": "4.8",
          "label": "4 / 8"
        },
        {
          "value": "9.3",
          "label": "9 / 3"
        },
        {
          "value": "3.9",
          "label": "3 / 9"
        },
        {
          "value": "10.2",
          "label": "10 / 2"
        },
        {
          "value": "2.10",
          "label": "2 / 10"
        }
      ]
    },
    {
      "type": "select",
      "id": "contentalign",
      "label": "Content vertical align",
      "default": "classnoactions",
      "options": [
        {
          "value": "tt-point-v-t",
          "label": "Top"
        },
        {
          "value": "classnoactions",
          "label": "Center"
        },
        {
          "value": "tt-point-v-b",
          "label": "Bottom"
        }
      ]
    },
    {
      "type": "select",
      "id": "textalign",
      "label": "Text align",
      "default": "center",
      "options": [
        {
          "value": "left",
          "label": "Left"
        },
        {
          "value": "center",
          "label": "Center"
        },
        {
          "value": "right",
          "label": "Right"
        }
      ]
    },
    {
      "type": "header",
      "content": "Desktop texts size"
    },
    {
      "type": "text",
      "id": "textoffset",
      "label": "Texts offset",
      "default": "60",
      "info": "integer, in px"
    },
    {
      "type": "range",
      "id": "fontsizetext1",
      "min": 14,
      "max": 100,
      "step": 1,
      "unit": "px",
      "label": "Font size text 1",
      "default": 32
    },
    {
      "type": "range",
      "id": "lineheighttext1",
      "min": 14,
      "max": 101,
      "step": 1,
      "unit": "px",
      "label": "Line height text 1",
      "default": 51
    },
    {
      "type": "range",
      "id": "fontweighttext1",
      "min": 300,
      "max": 900,
      "step": 100,
      "label": "Font weight text 1",
      "default": 700
    },
    {
      "type": "range",
      "id": "bottompaddingtext1",
      "min": 0,
      "max": 100,
      "step": 1,
      "unit": "px",
      "label": "Text 1 bottom padding",
      "default": 25
    },
    {
      "type": "range",
      "id": "fontsizetext2",
      "min": 14,
      "max": 100,
      "step": 1,
      "unit": "px",
      "label": "Font size text 2",
      "default": 22
    },
    {
      "type": "range",
      "id": "lineheighttext2",
      "min": 14,
      "max": 101,
      "step": 1,
      "unit": "px",
      "label": "Line height text 2",
      "default": 35
    },
    {
      "type": "range",
      "id": "fontweighttext2",
      "min": 300,
      "max": 900,
      "step": 100,
      "label": "Font weight text 2",
      "default": 400
    },
    {
      "type": "range",
      "id": "buttonpadding",
      "min": 0,
      "max": 100,
      "step": 1,
      "unit": "px",
      "label": "Button padding",
      "default": 20
    },
    {
      "type": "header",
      "content": "Mobile texts size"
    },
    {
      "type": "range",
      "id": "fontsizetext1_mobile",
      "min": 12,
      "max": 112,
      "step": 1,
      "unit": "px",
      "label": "Text 1 size mobile",
      "default": 20
    },
    {
      "type": "range",
      "id": "lineheighttext1_mobile",
      "min": 14,
      "max": 101,
      "step": 1,
      "unit": "px",
      "label": "Line height text 1 mobile",
      "default": 28
    },
    {
      "type": "range",
      "id": "bottompaddingtext1_mobile",
      "min": 0,
      "max": 100,
      "step": 1,
      "unit": "px",
      "label": "Text 1 bottom padding mobile",
      "default": 25
    },
    {
      "type": "range",
      "id": "fontsizetext2_mobile",
      "min": 12,
      "max": 112,
      "step": 1,
      "unit": "px",
      "label": "Text 2 size mobile",
      "default": 16
    },
    {
      "type": "range",
      "id": "lineheighttext2_mobile",
      "min": 14,
      "max": 101,
      "step": 1,
      "unit": "px",
      "label": "Line height text 2 mobile",
      "default": 24
    },
    {
      "type": "range",
      "id": "buttonpadding_mobile",
      "min": 0,
      "max": 100,
      "step": 1,
      "unit": "px",
      "label": "Button padding mobile",
      "default": 20
    },
    {
      "type": "header",
      "content": "Texts color"
    },
    {
      "type": "color",
      "id": "colortext1",
      "label": "Text 1 color",
      "default": "#191919"
    },
    {
      "type": "color",
      "id": "colortext2",
      "label": "Text 2 color",
      "default": "#777777"
    },
    {
      "type": "header",
      "content": "Background color"
    },
    {
      "type": "checkbox",
      "id": "usebgcolor",
      "label": "Use background color",
      "default": false
    },
    {
      "type": "color",
      "id": "bgco",
      "label": "Background color",
      "default": "#ffffff"
    },
    {
      "type": "text",
      "id": "bgdepad",
      "label": "Vertical top padding desktop",
      "default": "80",
      "info": "integer, in px"
    },
    {
      "type": "text",
      "id": "bgdepad_2",
      "label": "Vertical bottom padding desktop",
      "default": "80",
      "info": "integer, in px"
    },
    {
      "type": "text",
      "id": "bgmopad",
      "label": "Vertical top padding mobile",
      "default": "80",
      "info": "integer, in px"
    },
    {
      "type": "text",
      "id": "bgmopad_2",
      "label": "Vertical bottom padding mobile",
      "default": "80",
      "info": "integer, in px"
    },
    {
      "type": "paragraph",
      "content": "Background image"
    },
    {
      "type": "checkbox",
      "id": "showbgimage_bg",
      "label": "Show background image",
      "default": false
    },
    {
      "type": "image_picker",
      "id": "image_bg",
      "label": "Image"
    },
    {
      "type": "select",
      "id": "imagealign_bg",
      "label": "Image postion",
      "default": "center",
      "options": [
        {
          "value": "top",
          "label": "Top"
        },
        {
          "value": "center",
          "label": "Center"
        },
        {
          "value": "bottom",
          "label": "Bottom"
        }
      ]
    },
    {
      "type": "select",
      "id": "imagesize_bg",
      "label": "Image size",
      "default": "cover",
      "options": [
        {
          "value": "cover",
          "label": "Full background size"
        },
        {
          "value": "contain",
          "label": "Contain"
        }
      ]
    },
    {
      "type": "checkbox",
      "id": "usemiheight",
      "label": "Use minimal height",
      "default": false
    },
    {
      "type": "text",
      "id": "backgroundminheight3",
      "label": "Min height for big screen size",
      "default": "450",
      "info": "integer, in px"
    },
    {
      "type": "text",
      "id": "backgroundminheight2",
      "label": "Min height for medium screen size",
      "default": "450",
      "info": "integer, in px"
    },
    {
      "type": "text",
      "id": "backgroundminheight1",
      "label": "Min height for mobile screen size",
      "default": "200",
      "info": "integer, in px"
    }
  ],
  "presets": [
    {
      "name": "Text Plus Image",
      "category": "Simple Section"
    }
  ]
};
