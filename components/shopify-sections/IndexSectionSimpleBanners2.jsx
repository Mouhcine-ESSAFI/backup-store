// components/shopify-sections/IndexSectionSimpleBanners2.jsx
// Converted from Shopify section: index-section-simple_banners2

'use client';
import Image from 'next/image';

export default function IndexSectionSimpleBanners2({ settings, data }) {
  // Settings from Shopify:
  // Top Offset: select
  // Custom css classes: textarea
  // Fullwidth size: checkbox
  // Items In Line: select
  // Two in line in mobile: checkbox
  // undefined: header
  ...
  // Image bottom padding for texts: range
  // Show images as circle: checkbox
  // Use image maximum width: checkbox
  // Image maximum width: textarea
  // undefined: header
...
<div className="{item.top_offset == 'none' && (nomargin {% elsif item.top_offset == 'small' %}tt-offset-small )}container-indent {item.custom_classes} ttlayoutpromo02{section.id}">
  {item.usebgcolor && (<div>)}
  <div className="container{item.fullwidth && (-fluid-custom)} container-fluid-custom-mobile-padding">
    ...
    {%- endif -%}
    <div className="row tt-layout-promo02">
      {%- if section.blocks.size > 0 -%}
      {%- for block in section.blocks -%}{%- assign b_i = block.settings -%}
      <div className="col-sm-6 col-md-{item.items} {item.twoinmobile && (col-6{% else %}col-12-440width)} blid{block.id}" {block.shopify_attributes}>
        <div className="respimgsize tt-promo02{b_i.center_texts && ( text-center)}" data-hovercolors>
          <a href="{b_i.link}" className="image-box"
             {item.images_circle or item.use_img_width && ( style="{item.images_circle && (border-radius: 50%;)}{item.use_img_width && ( max-width: {item.img_width}px;)}")}
             >
          ...
          <div className="tt-description">
            {b_i.text1 != '' or b_i.text2 != '' && (
            <a href="{b_i.link}" className="tt-title">
              {b_i.text1 != '' && (<div className="tt-title-small"
                                           style="color:{b_i.text1c};"
                                           data-c="{b_i.text1c}"
                                           data-ac="{b_i.text1ca}"
                                           >{b_i.text1}</div>)}
              {b_i.text2 != '' && (<div className="tt-title-large"
            ...
            {b_i.text3 != '' && (<p className="promo2-text3" style="color:{b_i.text3c};">{b_i.text3}</p>)}
            {b_i.show_button && (<a href="{b_i.link}"
                                       className="btn"
                                       >{b_i.button_name}</a>)}
          </div>
        </div>
