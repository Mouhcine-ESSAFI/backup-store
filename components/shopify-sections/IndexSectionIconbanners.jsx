// components/shopify-sections/IndexSectionIconbanners.jsx
// Converted from Shopify section: index-section-iconbanners

'use client';
import Image from 'next/image';

export default function IndexSectionIconbanners({ settings, data }) {
  // Settings from Shopify:
  // Top Offset: select
  // Custom css classes: textarea
  // Fullwidth size: checkbox
  // Direction in banner is vertical: checkbox
...
<div className="{item.top_offset == 'none' && (nomargin {% elsif item.top_offset == 'small' %}tt-offset-small )}container-indent {item.custom_classes} iconbanners{section.id}">
  {item.usebgcolor and item.useboxedbgcolor == false && (<div className="subsectionbg">)}
  {item.usebgcolor and item.useboxedbgcolor && (<div className="container subsectionbg">{% else %}<div className="container{item.fullwidth && (-fluid)}">)}
    {%- if item.show_title -%}
    ...
    {%- endif -%}
    <div className="row{item.items_always_in_center && ( justify-content-center)} tt-services-listing{item.use_background && ( tt-layout-02)}"{item.use_background and item.background_boxed && ( style="background:{item.background_color}")}>
      {%- if section.blocks.size > 0 -%}
      {%- for block in section.blocks -%}{%- assign b_i = block.settings -%}
      <div className="col-xs-12 col-md-{item.items} col-lg-{item.items}" {block.shopify_attributes}>
        {%- if b_i.link == nil -%}<div className="tt-services-block{item.vertical && ( tt-services-block-vertical)}" data-hovercolors>{% else %}<a href="{b_i.link}" className="tt-services-block{item.vertical && ( tt-services-block-vertical)}" data-hovercolors>)}
          {%- if b_i.show_icon != 'none' -%}
          <div className="tt-col-icon">
            {%- if b_i.show_icon == 'image' -%}
            <img src="{{ b_i.image | img_url: '60x' }}" alt="{{ b_i.image.alt }}">
            {%- else -%}
            <i className="{b_i.icon}" style="color: {item.same_iconcolor && ({item.icon_color}{% else %}{b_i.icon_color})}"></i>
            {%- endif -%}
          </div>
          {%- endif -%}
          <div className="tt-col-description">
            <h4 className="tt-title" style="color:{item.title_color};" data-c="{item.title_color}" data-ac="{item.title_hover_color}">{b_i.text1}</h4>
            {% unless b_i.text2 == '' %}<p style="color: {item.desc_color}">{b_i.text2}</p>{% endunless %}
          </div>
        {%- if b_i.link == nil -%}</div>{% else %}</a>)}
      </div>
      {%- endfor -%}
      {%- else -%}
  ...
  {item.usebgcolor and item.useboxedbgcolor && (</div>{% else %}</div>)}
  {item.usebgcolor and item.useboxedbgcolor == false && (</div>)}
</div>
{item.usebgcolor or item.show_title && (
<style>
  {item.show_title && (
  {item.text1 != '' && (
