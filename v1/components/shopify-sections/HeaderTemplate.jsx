// components/shopify-sections/HeaderTemplate.jsx
// Converted from Shopify section: header-template

'use client';
import Image from 'next/image';

export default function HeaderTemplate({ settings, data }) {
  // Settings from Shopify:
  // Enable sticky header: checkbox
  // Small header height: checkbox
  // undefined: header
  // Menu Menu: link_list
  // undefined: header
  ...
  // undefined: header
  // Logo bar: select
  // Main menu position: select
  // Menu design: select
  // Menu hover underline: checkbox
  // "All in line". Menu center correction: text
  // Shrink main menu. Show items: text
...
{%- when 'allinline' -%}tt-col-obj tt-obj-logo{item.menu_bar == 'menucenter' && ( menu-in-center)}
{%- else -%}tt-obj-logo
{%- endcase -%}{%- endcapture -%}
...
<header className="desctop-menu-large{item.small_header_height && ( small-header)}{item.hoverunderline && ( tt-hover-03)}{item.logo_bar =="allinlinelogocenter" && ( parentallinlinelogocenter)}">
  ...
  <div className="tt-desktop-header{settings.header_position and template == 'index' && ( tt-header-static)}">
    ...
    <div className="container{item.header_fullwidth && (-fluid)}">
      <div className="tt-header-holder"{item.logo_bar =="allinlinelogocenter" && ( style="height: {item.headerheightcenter}px")}>
        ...
        <div className="tt-col-obj tt-obj-menu {{ menuposition }}"{item.menu_bar == 'menucenter' and item.menupadding != '' && ( style="padding-right: {item.menupadding}px;")}>
          <!-- tt-menu -->
    ...
    {%- if item.menu_design != 'tt-menu-small' -%}<div className="tt-color-scheme-01 menubackground">{%- endif -%}
      <div className="container{item.header_fullwidth && (-fluid)} single-menu">
        <div className="tt-header-holder">
          <div className="tt-obj-menu {{ menuposition }}">
            <div className="tt-desctop-parent-menu tt-parent-box">
              <div className="tt-desctop-menu {item.menu_design}">
                {%- include "main-menu" -%}
              </div>
            </div>
          </div>
