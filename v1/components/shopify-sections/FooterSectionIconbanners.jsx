// components/shopify-sections/FooterSectionIconbanners.jsx
// Converted from Shopify section: footer-section-iconbanners

'use client';
import Image from 'next/image';

export default function FooterSectionIconbanners({ settings, data }) {
  // Settings from Shopify:
  // Top Offset. Only for index page: select
  // Top Offset: select
...
<div className="{template == 'index' && ({item.top_offset}{% else %}{item.top_offset_normal})} {item.custom_classes} for-footer-blocks iconbanners{section.id}">
  ...
  {item.usebgcolor and item.useboxedbgcolor && (<div className="container subsectionbg">{% else %}<div className="container{item.fullwidth && (-fluid)}">)}
    ...
    {%- endif -%}
    <div className="row{item.items_always_in_center && ( justify-content-center)} tt-services-listing{item.use_background && ( tt-layout-02)}">
      {%- if section.blocks.size > 0 -%}
      {%- for block in section.blocks -%}{%- assign b_i = block.settings -%}
      <div className="col-xs-12 col-md-6 col-lg-{item.items}" {block.shopify_attributes}>
        <a href="{b_i.link}" className="tt-services-block{item.vertical && ( tt-services-block-vertical)}" data-hovercolors>
