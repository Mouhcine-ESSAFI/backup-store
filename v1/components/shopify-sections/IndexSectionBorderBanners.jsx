// components/shopify-sections/IndexSectionBorderBanners.jsx
// Converted from Shopify section: index-section-border_banners

'use client';
import Image from 'next/image';

export default function IndexSectionBorderBanners({ settings, data }) {
...
<div className="{item.top_offset == 'none' && (nomargin {% elsif item.top_offset == 'small' %}tt-offset-small )}container-indent">
  <div className="container{item.fullwidth && (-fluid-custom)}">
    ...
    <div className="tt-block-title{item.title_to_left && ( text-left)}{item.boxed_title and item.fullwidth && ( container)}">
      <h1 className="tt-title">{item.text1}</h1>
      {item.text2 != '' && (<div className="tt-description">{item.text2}</div>)}
    </div>)}
    {%- if section.blocks.size > 0 -%}
    <div className="row tt-img-box-listing">
      {%- for block in section.blocks -%}{%- assign b_i = block.settings -%}
      <div className="col-6 col-sm-4 col-md-{{ grid }} col-12-440width" {block.shopify_attributes}>
        <a href="{b_i.link}" className="tt-img-box" style="border-color:{item.border_base_color};" data-borc="{item.border_base_color}" data-aborc="{item.border_active_color}" data-hovercolors>
