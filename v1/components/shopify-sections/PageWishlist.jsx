// components/shopify-sections/PageWishlist.jsx
// Converted from Shopify section: page-wishlist

'use client';
import Image from 'next/image';

export default function PageWishlist({ settings, data }) {
...
<div className="container-indent nomargin">
  <div className="tt-empty-cart">
    <div className="tt-block-title">
      <h1 className="tt-title">{{ "wishlist.wishlist_page.title" | t }}</h1>
    </div>
    <div className="tt-login-wishlist">
      <p>{{ "wishlist.wishlist_page.login_message" | t }}</p>
      <div className="row-btn">
        <a href="/account/login" className="btn btn-small ttbtnmainstyle">{{ "wishlist.wishlist_page.sign_in_button" | t }}</a>
        <a href="/account/register" className="btn btn-border btn-small">{{ "wishlist.wishlist_page.register_button" | t }}</a>
      </div>
    </div>
  </div>
</div>
...
<div className="container-indent nomargin wkpagenoinfo{% unless customer.metafields.wishlist == blank %} hide{% endunless %}">
  <div className="tt-empty-wishlist">
    ...
    <a href="/" className="btn ttbtnmainstyle">{{ "wishlist.wishlist_page.continue_shopping" | t }}</a>
  </div>
</div>
{% unless customer.metafields.wishlist == blank %}
<div className="container-indent wkpageinfo">
