// components/shopify-sections/HeaderTemplate4.jsx
// Converted from Shopify section: header-template-4

'use client';
import Image from 'next/image';

export default function HeaderTemplate4({ settings, data }) {
  // Settings from Shopify:
  // undefined: header
  // undefined: paragraph
  // Show menu with icons: checkbox
  // Menu with icons fullwidth: checkbox
  // undefined: header
  ...
  // undefined: header
  // Show account links: checkbox
  // Text between account links: text
  // Show options links: checkbox
  // Show slogan: checkbox
  // Center slogan in the header: checkbox
  // Slogan: textarea
  // undefined: header
  // Instagram url: url
  // Compare text: text
...
<header className="desctop-menu-large headertype3 headertype4">
  ...
  <div className="tt-desktop-header">
    <div className="tt-wrapper-light">
      ...
      <div className="headertype4_box1 headertype4_bottomline">
        <div className="container{item.header_fullwidth && (-fluid)} tt-zindex5">
          <div className="tt-header-holder">
            ...
            <div className="tt-desctop-parent-menu tt-parent-box">
              <div className="tt-desctop-menu d-flex justify-content-center">
                {%- include "main-menu" -%}
              </div>
            </div>
            ...
            <div className="tt-col-obj obj-move-right header-inline-options-box">
              ...
              <div className="tt-parent-box tt-desctop-parent-headtype1">
                <div className="tt-search-opened-headtype1 headerformplaceholderstyles my-1">
                  <form action="/search" method="get" role="search">
      ...
      <div className="headertype4_box2 headertype4_bottomline">
        <div className="container{item.header_fullwidth && (-fluid)}">
          <div className="tt-header-holder{item.show_simple_info and item.center_simple_info && ( center-header-tel-info)}">

            <div className="tt-obj-logo" itemscope itemtype="http://schema.org/Organization">
              {%- include "header_logo" -%}
            </div>

            {%- if item.show_simple_info and item.center_simple_info -%}
            <span className="header-tel-info">
              {item.simple_info}
            </span>
            {%- endif -%}
            ...
            <div className="tt-col-obj tt-obj-options obj-move-right">
              ...
              {%- if item.show_simple_info and item.center_simple_info == false -%}
              <span className="header-tel-info">
                {item.simple_info}
              </span>
              {%- endif -%}
              ...
              <div className="h-account-details">
                ...
                {item.customer_links_text} <a href="/account/logout">{{ "layout.customer.log_out" | t }}</a>
                {%- else -%}<a href="/account/login">{{ "layout.customer.sign_in" | t }}</a>
                {item.customer_links_text} <a href="/account/register">{{ "layout.customer.register" | t }}</a>
                {%- endif -%}
                {%- endif -%}
              </div>
              {%- endif -%}

              {%- if settings.show_header_insta -%}
              <div className="header-icon-with-text tt-wishlist">
                {item.insta_link != '' && (<a href="{item.insta_link}" data-tooltip="{{ "general.tooltip_texts.instagram_account" | t }}" data-tposition="bottom" target="_blank">)}
                  <span className="tt-icon m-0">
                    {% include "svg-insta" %}
                  </span>
                {item.insta_link != '' && (</a>)}
              </div>
    ...
    {%- if item.show_categorie_button -%}
    <div className="headertype4_box3">
      <div className="container{item.header_icons_fullwidth && (-fluid)} menu-icons-box tostuck-det-js" data-parent="menu-icons-box" data-stuckparent="tt-stuck-parent-menu-icons-box">
  ...
  <!-- stuck nav -->
  <div  {template == "product" && (style="top: -100% !important;")} className="tt-stuck-nav{% unless item.header_sticky_header %} disabled{% endunless %}{settings.use_header_inline && ( notshowinmobile)}">
    <div className="container{item.header_fullwidth && (-fluid)}">
      <div className="tt-header-row ">
    ...
    {%- if item.show_categorie_button -%}
    <div className="container{item.header_icons_fullwidth && (-fluid)}">
      <div className="tt-stuck-parent-menu-icons-box fromstuck-det-js"></div>
    </div>
    {%- endif -%}
...
export const schema = {
  ...
  "settings": [
    ...
    {
      "type": "paragraph",
      "content": "After saving the page with \"Menu with icons\" you will see the changes!!!"
    },
    {
      "type": "checkbox",
      "id": "show_categorie_button",
      "label": "Show menu with icons",
      "default": true,
      "info": "Do action and save page!"
    },
    {
      "type": "checkbox",
      "id": "header_icons_fullwidth",
      "label": "Menu with icons fullwidth",
      "default": false
    },
    {
      "type": "header",
    ...
    {
      "type": "text",
      "id": "customer_links_text",
      "label": "Text between account links",
      "default": "or"
    },
    {
      "type": "checkbox",
    ...
    {
      "type": "checkbox",
      "id": "center_simple_info",
      "label": "Center slogan in the header",
      "default": false,
      "info": "Absolute center only for screen size > 1500px"
    },
    {
      "type": "textarea",
      "id": "simple_info",
