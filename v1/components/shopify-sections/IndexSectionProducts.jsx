// components/shopify-sections/IndexSectionProducts.jsx
// Converted from Shopify section: index-section-products

'use client';
import Image from 'next/image';

export default function IndexSectionProducts({ settings, data }) {
  // Settings from Shopify:
  // Top Offset: select
  // Custom css classes: textarea
  // Fullwidth size: checkbox
  // No paddings: checkbox
  // Hide all elements after image: checkbox
  ...
  // undefined: header
  // Use title as link to the collection: checkbox
  // Show button: checkbox
  // Button text: text
  // undefined: header
  // Collection: collection
  // Products in center: checkbox
  // Items Length. Integer: text
  // Products In Row: select
  // undefined: header
  // Default Carousel: checkbox
  // Boxed buttons when fullwidth: checkbox
  // Show one product in mobile: checkbox
  // Use slider adaptive height: checkbox
  // Items in row: select
...
<div className="{item.top_offset == 'none' && (nomargin {% elsif item.top_offset == 'small' %}tt-offset-small )}container-indent sectionproducts{section.id} {item.custom_classes}">
  {item.nopaddings && (<div className="nopaddingspr">)}
  {usebgcolor && (<div className="products-section-bg"{showbgimage && ( style="background-image: url({useimage});")}>)}
  <div className="container{item.fullwidth && (-fluid-custom)} container-fluid-custom-mobile-padding" data-sectionname="index_sectionproducts">
    {%- if item.show_title -%}<div className="tt-block-title{item.title_to_left && ( text-left)}{item.boxed_title and item.fullwidth && ( container)}">
      <h2 className="tt-title">
        {%- if item.use_title_as_link -%}<a href="/collections/{collection.handle}" className="title-hover-underline" title="{item.button_text}">{item.text1}</a>
        {%- else -%}{item.text1}
        {%- endif -%}
      </h2>
    ...
    <div className="{item.boxed_buttons and item.fullwidth && (slboxedbuttons )}tt-carousel-products-js row arrow-location-tab tt-alignment-img tt-layout-product-item slick-animated-show-js" data-slick='{"slidesToShow": {item.toshow}, "slidesToScroll": {item.step}, "autoplay": {item.module_autoplay}, "autoplaySpeed": {item.delay_time}}'
         data-mobile="{item.show_oneproduct_in_mobile && (1{% else %}2)}" 
         data-adaptive="{item.use_adaptive_height && (true{% else %}false)}">
      {%- for product in collection.products limit: length -%}
      <div className="forsliders-margins">
        {%- include "product-grid-item" -%}
      </div>
      {%- endfor -%}
    </div>
    {%- else -%}
    <div className="row tt-layout-product-item{item.items_always_in_center && ( justify-content-center)}">
      {%- for product in collection.products limit: length -%}
      <div className="col-6 col-md-4 {grid == '5' && (col-5-inrow-lg{% else %}col-lg-{{ grid }})}">
        {%- include "product-grid-item" -%}
      </div>
      {%- endfor -%}
    </div>
    ...
    <div className="text-center">
      <a href="/collections/{collection.handle}" className="btn btn-border btn-top">{item.button_text}</a>
    </div>
    {%- endif -%}
