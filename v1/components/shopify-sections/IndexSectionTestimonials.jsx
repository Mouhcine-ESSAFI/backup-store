// components/shopify-sections/IndexSectionTestimonials.jsx
// Converted from Shopify section: index-section-testimonials

'use client';
import Image from 'next/image';

export default function IndexSectionTestimonials({ settings, data }) {
  ...
  // undefined: paragraph
  // Change buttons settings: checkbox
  // Background base color: color
  // Arrow base color: color
  // Background active color: color
  // Arrow active color: color
  // Buttons vertical aligment. In percent: text
  // undefined: header
  // Use background color: checkbox
  // Boxed background: checkbox
  // Mobile bottom padding: checkbox
  ...
  // Image: image_picker
  // Use background minimal height: checkbox
  // Background minimal height in px. Desktop: text
  // Background minimal height in px. Mobile: text
...
{%- capture bgco =  -%}
{section.settings.usebgcolor && (
style="background: {settings.bgco};
{section.settings.usebgimage && (
...
<style>
  ...
  .testimonials{section.id} .testimonials-bg-pad{
    padding-top: {settings.bgdepad}px;
    padding-bottom: {settings.bgdepad_2}px;
  }
  @media (max-width: 768px){
    .testimonials{section.id} .testimonials-bg-pad{
      padding-top: {settings.bgmopad}px;
      padding-bottom: {settings.bgmopad_2}px;
    }
  }
  {section.settings.usebgminheight && (
  ...
  @media (min-width: 769px){
    .testimonials{section.id}{
      min-height: {settings.bgminheight}px;
    }
  }
  @media (max-width: 768px){
    .testimonials{section.id}{
      min-height: {settings.bgminheight_m}px;
    }
  }
...
<div className="{item.top_offset == 'none' && (nomargin {% elsif item.top_offset == 'small' %}tt-offset-small )}container-indent testimonials{section.id}"{section.settings.boxedbg == false && ( {{ bgco }})}>
  <div className="container{section.settings.usebgcolor and section.settings.boxedbg == false && ( testimonials-bg-pad{section.settings.mobpad && ( testimonials-mob-pad)})}" data-sectionname="index_testimonials">
    {section.settings.boxedbg && (<div className="col-12{section.settings.usebgcolor and section.settings.boxedbg && ( testimonials-bg-pad{section.settings.mobpad && ( testimonials-mob-pad)})}" {{ bgco }}>)}
    {%- if section.settings.text1 != "" -%}
    <div className="tt-block-title">
      <h2 className="tt-title" style="color: {settings.titleco};">{settings.text1}</h2>
    </div>
    {%- endif -%}
    {%- if section.blocks.size > 0 -%}
    <div className="tt-slider-fullwidth arrow-location-center-02 slick-animated-show-js" data-slick='{"slidesToShow": 1, "slidesToScroll": 1, "autoplay": {item.module_autoplay}, "autoplaySpeed": {item.delay_time}}'>
      {%- for block in section.blocks -%}{%- assign b_i = block.settings -%}
      <div className="item" {block.shopify_attributes}>
        <a href="{{ block.settings.link }}" className="tt-content-info obj-data02{block.id}">
          ...
          {% unless b_i.text1 == '' %}<h3 className="tt-title" style="color: {settings.rtitleco};">{b_i.text1}</h3>{% endunless %}
          {% unless b_i.text2 == '' %}<p style="color: {settings.descco};">{b_i.text2}</p>{% endunless %}
          {b_i.text3 != '' or b_i.text4 != '' or b_i.usemedia && (
          <div className="tt-subscription">
            ...
            {% unless b_i.text3 == '' %}<div className="tt-text-lage" style="color: {settings.authco};">{b_i.text3}</div>{% endunless %}
            {% unless b_i.text4 == '' %}<div className="tt-text-small" style="color: {settings.hobbyco};">{b_i.text4}</div>{% endunless %}
          </div>
          {%- endif -%}
        </a>
      </div>
    ...
    {%- endif -%}
    {section.settings.boxedbg && (</div>)}
  </div>
  {section.settings.changebntcolor && (
  <style>
    .testimonials{section.id} button{
      background: {settings.bgbaseco} !important;
      color: {settings.arrbaseco} !important;
    }
    .testimonials{section.id} button:hover{
      background: {settings.bgactiveco} !important;
      color: {settings.arractiveco} !important;
    }
    .arrow-location-center-02 .slick-arrow, .arrow-location-center-02 .slick-arrow:active {
      top: {settings.btnsposition}% !important;
    }
  </style>
  )}
</div>
